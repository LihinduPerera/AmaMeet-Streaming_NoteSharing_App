...
class ClassNotesBloc extends Bloc<ClassNotesEvent, ClassNotesState> {
  ...
  Future<void> _onLoad(LoadClassNotes event, Emitter emit) async {
    ...
    _notesSub = _repo.notesStreamForClass(event.classId).listen((notes) {
      add(_ClassNotesUpdated(notes));
    }, onError: (e) => add(_ClassNotesError(e.toString())));
  }

  Future<void> _onUpload(UploadClassNoteEvent e, Emitter emit) async {
    try {
      await _repo.uploadClassNote(classId: e.classId, file: e.file, filename: e.filename);
  ...
  Future<void> _onUpdate(UpdateClassNoteEvent e, Emitter emit) async {
    try {
      await _repo.updateClassNote(docId: e.docId, file: e.file, filename: e.filename);
  ...
  // internal events used by stream
  void _onInternalEvents() {
    // no-op here: see _ClassNotesUpdated below
  }
